...
const svgCC = {
			...
			if (node.tag === 'foreignObject') {
				...
				if (svgChild && svgChild.attrs?.['iftool-background'] && svgChild.children.length === 0 && parentInfo?.parentNode?.children) {
					...
					const imageNode = {
						...
						attrs: {
							...preservedAttrs,
							'iftool-href': svgChild.attrs['iftool-background'],
							x: imageX,
							y: imageY,
							width: imageWidth,
							height: imageHeight,
			...
			if (node.tag === 'svg') {
				...
				if (parentIsNotFo && node.attrs?.['iftool-background'] && parentInfo?.parentNode?.children) {
					...
					const imageNode = {
						tag: 'image',
						attrs: {
							'iftool-href': node.attrs['iftool-background'],
							x: '0',
							y: '0',
							width: imageWidth,
							height: imageHeight,
	...
	image2svg: function(tree) {
		...
		this.traverseHtmlTree(tree, (node, parentInfo) => {
			...
			if (node.tag === 'image' && node.children.length === 0 && node.attrs?.['iftool-href'] && parentInfo?.parentNode?.children) {
				...
				// viewBox仅接受数值，宽高为百分比时按1:1兜底（公众号标准宽1080）
				const vbWidth = /^\d+(\.\d+)?$/.test(String(imageWidth)) ? imageWidth : '1080';
				const vbHeight = /^\d+(\.\d+)?$/.test(String(imageHeight)) ? imageHeight : '1080';
	...
	img2image: async function(tree) {
		...
		// 并发池处理：节点替换基于预存索引互不依赖可安全并发，限制并发数避免同时加载过多图片
		const concurrencyLimit = 5;
		const imgQueue = [...targetImgs];
		const processImgItem = async (item) => {
			...
			try {
				...
				const defaultNode = {
					tag: 'svg',
					attrs: { viewBox: '0 0 1080 1080', style: 'display: block; pointer-events: painted; width: 100%;' }, // viewBox仅接受数值，按1:1宽高比兜底
					children: [{
						tag: 'image',
						attrs: {
							...preservedAttrs,
		...
		const imgWorkers = Array.from({ length: Math.min(concurrencyLimit, imgQueue.length) }, async () => {
			while (imgQueue.length > 0) {
				await processImgItem(imgQueue.shift());
			}
		});
		...
		const processImgItem = async (item) => {
			...
			try {
				...
				const defaultSvgNode = {
					...
					attrs: {
						...preservedAttrs,
						viewBox: '0 0 1080 1080', // viewBox仅接受数值，按1:1宽高比兜底
						style: node.attrs.style || '',
						'iftool-background': node.attrs['iftool-src']
					},
					children: []
	...
	preprocess: function(code) {
		...
		/**
		 * 预处理0：替换所有div标签为section标签
		 * @param {string} html - 原始HTML字符串
		 * @returns {string} 修复后的HTML字符串
		 */
		const replaceDiv = (html) => {
			return html.replace(/<\/?div/g, match => match === '<div' ? '<div-iftool' : '</div-iftool');
		};
		...
		const escapeAmpersands = (html) => {
			const validEntities = 'amp|lt|gt|quot|nbsp|copy|yen|euro|deg|times|permil|#[0-9]+|#[xX][0-9a-fA-F]+'; // 含数字实体，避免&#160;等被二次转义
			return html.replace(
		...
		const fixCase = (html) => {
			...
			const normalizeAttrs = (str) => {
				...
				const attrRegex = new RegExp(`([\\s>])(${attrKeys})\\s*=`, 'gi');
		...
		/**
		 * 预处理3.5：移除公众号禁止的内容（script/style标签、on*事件属性等一切JS痕迹）
		 * @param {string} html - 原始HTML字符串
		 * @returns {string} 清理后的HTML字符串
		 */
		const removeForbidden = (html) => {
			let out = html.replace(/<(script|style)\b[\s\S]*?<\/\1>/gi, ''); // 成对的script/style标签及内容
			out = out.replace(/<(script|style)\b[^>]*?\/?>/gi, ''); // 残留的单标签/自闭合
			out = out.replace(/\s+on[a-z]+\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, ''); // on*事件属性
			return out;
		};
	...
	extractAssets: function(rootNode) {
		...
		while (node = walker.nextNode()) {
			// 提取image上的href图片链接（兼容旧代码xlink:href写法），设置为iftool-href
			if (node.tagName.toLowerCase() === 'image') {
				const imageHref = node.getAttribute('href') || node.getAttribute('xlink:href');
				if (imageHref) {
					node.setAttribute('iftool-href', imageHref);
	...
	compose: function(tree) {
		...
		xmlString = xmlString.replace(/div-iftool/g, 'div'); // 还原全部被转义的div标签（原写法只替换第一个，会残留残损闭合标签）
		const rootMatch = xmlString.match(/<div id="iftool">([\s\S]*)<\/div>\s*$/); // 贪婪匹配至字符串末尾，确保取到包裹容器自身的闭合标签
		if (!rootMatch) {
			throw new Error('代码合成失败：未找到预处理包裹的根容器');
		}
		xmlString = rootMatch[1]; // 移除预处理时包裹的div#iftool容器
